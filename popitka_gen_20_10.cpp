...
void calculate_dannue()
{
	...
	for (int i = 0; i < parametr.size(); ++i)
		global_ans.push_back(atalon_function(parametr[i]));
...
struct network {
	...
	void update_delta() {
		...
		for (int i = 0; i < value.back().size(); ++i)
			delta.back()[i] = value.back()[i] - ans[i];
	...
	long long learn(vector<long double> v, vector<long double> _ans)
	{
		setAnswer(_ans);
		setInput(v);
		calculate_network();
...
int main()
{
	...
	while (true)
	{
		...
		{
			int buf;
			buf = diana.learn({ parametr[i] }, { global_ans[i] });
			//cout << buf << "\t";
			gr += buf;
			//diana.print_network();
		}
